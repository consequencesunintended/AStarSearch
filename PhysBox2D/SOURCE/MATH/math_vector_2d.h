...
public:
	...
	float GetReciprocalLength(void) const
	{
	...
	{
		const float new_x = cosf(radius) * X - sinf(radius) * Y;
		const float new_y = sinf(radius) * X + cosf(radius) * Y;

		X = new_x;
		Y = new_y;
	}

	void RotateAroundOther( float rad, const MATH_VECTOR_2D& other )
	{
		const float sin = sinf( rad );
		const float cos = cosf( rad );

		// subtract pivot point
		const float x_centered = X - other.X;
		const float y_centered = Y - other.Y;

		// rotate
		const float new_x = x_centered * cos - y_centered * sin;
		const float new_y = x_centered * sin + y_centered * cos;
		...
		//const float new_x = other.X + ( X - other.X ) * cosf( rad ) - ( Y - other.Y ) * sinf( rad );
		//const float new_y = other.Y + ( X - other.X ) * sinf( rad ) + ( Y - other.Y ) * cosf( rad );
